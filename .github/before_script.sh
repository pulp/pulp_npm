...
export PRE_BEFORE_SCRIPT=$GITHUB_WORKSPACE/.github/pre_before_script.sh
export POST_BEFORE_SCRIPT=$GITHUB_WORKSPACE/.github/post_before_script.sh
...
if [[ -f .github/pulp-smash-config.json ]]; then
    sed "s/localhost/$(hostname)/g" .github/pulp-smash-config.json > ~/.config/pulp_smash/settings.json
else
