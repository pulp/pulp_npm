...
if [ "$TEST" = 'bindings' ]; then
  python $GITHUB_WORKSPACE/.travis/test_bindings.py
  cd ../pulp-openapi-generator
  if [ ! -f $GITHUB_WORKSPACE/.travis/test_bindings.rb ]
  then
    exit
  fi
  ...
  ruby $GITHUB_WORKSPACE/.travis/test_bindings.rb
  exit
fi
...
export PYTHONPATH=$GITHUB_WORKSPACE:$GITHUB_WORKSPACE/../pulpcore:${PYTHONPATH}
...
if [[ "$TEST" == "performance" ]]; then
  ...
  if [[ -z ${PERFORMANCE_TEST+x} ]]; then
    pytest -vv -r sx --color=yes --pyargs --capture=no --durations=0 pulp_npm.tests.performance || show_logs_and_return_non_zero
  else
    pytest -vv -r sx --color=yes --pyargs --capture=no --durations=0 pulp_npm.tests.performance.test_$PERFORMANCE_TEST || show_logs_and_return_non_zero
  fi
  exit
fi
...
if [ -f $FUNC_TEST_SCRIPT ]; then
    ...
    pytest -v -r sx --color=yes --pyargs pulp_npm.tests.functional || show_logs_and_return_non_zero
fi
