...
then
  ...
  docker exec pulp pip3 install pulp-npm-client==$VERSION
  mkdir -p dist
...
find . -name "*.whl" -exec docker exec pulp pip3 install /root/pulp-openapi-generator/pulp_npm-client/{} \;
...
tar cvf ../../pulp_npm/python-client-docs.tar ./docs
exit $?
