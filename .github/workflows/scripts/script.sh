...
pushd ../pulp-openapi-generator
  ...
  do
    ...
    echo "/root/pulp-openapi-generator/${PACKAGE}-client/dist/${PACKAGE}_client-${VERSION}-py3-none-any.whl" >> "../pulp_npm/bindings_requirements.txt"
  done
popd
...
# Bindings diff for npm
jq '(.paths[][].parameters|select(.)) |= sort_by(.name)' < "npm-api.json" > "build-api.json"
jq '(.paths[][].parameters|select(.)) |= sort_by(.name)' < "../pulp-openapi-generator/npm-api.json" > "test-api.json"
...
if [[ "$TEST" == "performance" ]]; then
  if [[ -z ${PERFORMANCE_TEST+x} ]]; then
    cmd_user_prefix bash -c "pytest -vv -r sx --color=yes --suppress-no-test-exit-code --capture=no --durations=0 --pyargs pulp_npm.tests.performance"
  else
    cmd_user_prefix bash -c "pytest -vv -r sx --color=yes --suppress-no-test-exit-code --capture=no --durations=0 --pyargs pulp_npm.tests.performance.test_${PERFORMANCE_TEST}"
  fi
  exit
fi
...
if [ -f "$FUNC_TEST_SCRIPT" ]; then
  ...
  then
    ...
    cmd_user_prefix bash -c "pytest -v -r sx --color=yes --suppress-no-test-exit-code --pyargs pulp_npm.tests.functional -m 'not parallel' --nightly"
  else
    ...
    cmd_user_prefix bash -c "pytest -v -r sx --color=yes --suppress-no-test-exit-code --pyargs pulp_npm.tests.functional -m 'not parallel'"
  fi
fi
