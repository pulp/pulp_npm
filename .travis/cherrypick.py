...
def blobs_match(commit1, commit2):
    ...
    for cf in commit1.files:
        files = [
            f for f in commit2.files if f.sha == cf.sha and f.filename == cf.filename
        ]
        if len(files) == 0:
            return False
...
for issue in issues:
    ...
    for commit in get_merged_commits(pr):
        ...
        ret = subprocess.run(
            ["git", "cherry-pick", "-x", commit.sha], stderr=subprocess.PIPE
        )
...
pr = grepo.create_pull(
    f"Cherry picks to {STABLE_BRANCH}", body, STABLE_BRANCH, cherry_pick_branch
)
